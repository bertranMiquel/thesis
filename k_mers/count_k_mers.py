...
def count_k_mers(df_k_mers: pd.DataFrame, output_folder='../Data/Intermediate/k_mers/') -> None:
    ...
    df_k_mers_scaffold = df_k_mers.groupby(['species_name', 'replicon_accession', 'k'], as_index=False).count()
    df_k_mers_species = df_k_mers_scaffold.groupby(['species_name', 'k'], as_index=False).sum()

    df_k_mers_scaffold = df_k_mers_scaffold[['species_name', 'replicon_accession', 'k', 'k_mer']].rename(columns = {'k_mer': 'total_k_mers'})
    df_k_mers_species = df_k_mers_species[['species_name', 'k', 'k_mer']].rename(columns = {'k_mer': 'total_k_mers'})

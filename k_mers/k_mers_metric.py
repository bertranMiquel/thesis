...
def shared_k_mers(origin: str, level: str, df_k_mers: pd.DataFrame) -> pd.DataFrame:
    ...
    if level == 'species':
        df_k_mers = df_k_mers[['k', 'k_mer', level]]
    else:
        df_k_mers = df_k_mers[['k', 'k_mer', level, 'species']]

    # Get the k-mers of the origin scaffold or species
    df_origin = df_k_mers[df_k_mers[level] == origin].drop_duplicates()
    ...
    df_common_k_mers = df_shared.groupby(['k', level]).size().reset_index()
    df_common_k_mers = df_common_k_mers.rename(columns = {0:'common_k_mers'})
...
def k_mers_to_metric(origin: str, level: str, k_mers_counter: pd.DataFrame, df_length: pd.DataFrame, df_k_mers: pd.DataFrame) -> pd.DataFrame:
    ...
    df_common_k_mers.to_csv('../Data/Intermediate/df_common_k_mers.csv', index=False)

    k_mers_counter = k_mers_counter.merge(df_common_k_mers, on=['k', level], how='left')
    ...
    return df_metric.sort_values(by=['species', 'metric'], ascending=[True, False])